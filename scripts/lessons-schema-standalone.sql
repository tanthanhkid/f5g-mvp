-- ========================================
-- LESSONS SCHEMA FOR FREEDOM TRAINING (STANDALONE VERSION)
-- ========================================
...
-- 3. User Lesson Progress Table (WITHOUT users foreign key for standalone testing)
CREATE TABLE IF NOT EXISTS user_lesson_progress (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID NOT NULL, -- Will add foreign key constraint later when users table exists
    lesson_id UUID NOT NULL REFERENCES lessons(id) ON DELETE CASCADE,
...
-- Note: User TUTE points update trigger will be added later when users table is available 